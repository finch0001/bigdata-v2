...
public class StrParser {
    ...
    public void expect(String s) throws Exception {
        ...
        if (val.regionMatches(pos, s, 0, slen)) {
            ...
            throw new Exception("Expected '" + s + "' at position " + pos + " in '" + val + "'");
        }
    }
    ...
    public String getId(String errMessage) throws Exception {
        eatws();
        int id_start = pos;
        char ch;
...
//          if (!Character.isJavaIdentifierPart(ch) && ch != '.' && ch != ':') {
        ...
        if (errMessage != null) {
            throw new Exception(errMessage + " at pos " + pos + " str='" + val + "'");
        }
        return null;
    }

    public String getGlobbedId(String errMessage) throws Exception {
        eatws();
        int id_start = pos;
        char ch;
     ...
     */
    public Boolean getSortDirection() throws Exception {
    ...
    public String getQuotedString() throws Exception {
        eatws();
        char delim = peekChar();
        if (!(delim == '\"' || delim == '\'')) {
            return null;
        ...
        for (; ;) {
            if (pos >= end) {
                throw new Exception("Missing end quote for string at pos " + (val_start - 1) + " str='" + val + "'");
            }
            char ch = val.charAt(pos);
            if (ch == '\\') {
                pos++;
                ...
                switch (ch) {
                    ...
                    case 'u':
                        if (pos + 4 >= end) {
                            throw new Exception("bad unicode escape \\uxxxx at pos" + (val_start - 1) + " str='" + val + "'");
                        }
